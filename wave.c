...
char *generate_saw(unsigned long freq,unsigned long amp,char **p_buffer){
	...
	err = alsa_get_period_size(&periodsize);
	debug("period size: %lu",periodsize);
	check_err("alsa_get_period_size");
	...
	/* Number of points sampled,or number of samples in a period = period/sample period or sample rate / frequency of wave
	 * the signal varies between -1.0 and 1.0,so the step increment equals (1.0 - (-1.0)) / samples per period
	 */
	unsigned long frames; 
	unsigned int points_per_period = rate / freq;
	float step = 2.0 / points_per_period; 
	static float fsample = -1.0;

	for(frames = 0;frames < periodsize;frames++){

		debug("fsample: %f",fsample);
		fsample += step;
		if(fsample > 1.0){
			fsample = -1.0;
		}

		unsigned short sample = fsample * amp;

		buffer[2*frames + 0] = sample & 0x00ff;
		buffer[2*frames + 1] = (sample & 0xff00) >> BITS_PER_BYTE;
	}
...
char *generate_square(unsigned long freq,unsigned long amp,char **p_buffer){
	...
	/* When the number of samples written reaches half the number of points per wave period,
	 * The state variable is complimented(Multiplied by -1)
	 */
	unsigned long frames;
	static char state = 1;//Either 1 or -1
	unsigned int points_per_period = rate / freq;
	static unsigned int point = 0;

	for(frames = 0;frames < periodsize;frames++){
	
		debug("point: %u\tstate: %d",point,state);
		if(++point > points_per_period / 2){
			point = 0;
			state *= -1;
		}

		buffer[2*frames + 0] = (amp * state) & 0x00ff;
		buffer[2*frames + 1] = ((amp * state) & 0xff00) >> BITS_PER_BYTE;
	}
